...
class review_feature:
        ...
        if self.hindi_swear_words == None:
            ...
            try:
                with open(os.path.abspath('utils')+'/DictionaryUtils/hindi_swear_words.txt', 'r') as fp:
                    data = fp.read().lower()
                result = data.split('\n')
                self.hindi_swear_words = set(
                    [x.split('~')[0][:-1].lower() for x in result])
            except:
    ...
    def gibberish_detection(self, l, prefix_path='./'):
        ...
        if self.model_data == None:
            self.model_data = pickle.load(
                open(os.path.abspath(prefix_path)+'/gib_model.pki', 'rb'))
    ...
    def corpus_stem_lemma(self, corpus):
        ...
        for i in range(len(corpus)):
            doc = nlp(corpus[i])
            corpus[i] = " ".join([stemmer.stem(token.lemma_) for token in doc if token.is_stop ==
                                 False and token.is_punct == False and token.is_alpha == True])
            # print(temp[i])
        return corpus
    ...
    def noun_score(self, corpus):
        ...
        for review in corpus:
            doc = nlp(review)
            noun_tag.append([stemmer.stem(token.lemma_) for token in doc if token.pos_ ==
                            "NOUN" and token.is_stop == False and token.is_punct == False and token.is_alpha == True])

        tfidf = TfidfVectorizer(min_df=1, ngram_range=(1, 1))

        features = tfidf.fit_transform(corpus)

        # Access the feature names using the get_feature_names() attribute
        feature_names = tfidf.get_feature_names_out()

        reviews_tfidf = pd.DataFrame(features.todense(), columns=feature_names)
        reviews_tfidf['sum'] = reviews_tfidf.sum(axis=1)

        reviews_tfidf['noun_sum'] = 0.0
        reviews_tfidf['tfidf_score'] = 0.0

        for i in range(len(noun_tag)):
            sm = 0.0
            for q in noun_tag[i]:
                if q in reviews_tfidf.columns:
                    sm += reviews_tfidf[q][i]
            reviews_tfidf.at[i, 'noun_sum'] = sm
            if reviews_tfidf.at[i, 'sum'] == 0.0:
                reviews_tfidf.at[i, 'tfidf_score'] = 0.0
                continue
            reviews_tfidf.at[i, 'tfidf_score'] = float(
                reviews_tfidf.at[i, 'noun_sum'] / reviews_tfidf.at[i, 'sum'])

        return reviews_tfidf['tfidf_score']
